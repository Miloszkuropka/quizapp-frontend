...
function EditUser() {
    ...
    const { token, user, supervisor } = useContext(LoginContext); // Pobranie tokenu i ról użytkownika
    const [userDetails, setUserDetails] = useState(null);
    const [loading, setLoading] = useState(false);
    const [updating, setUpdating] = useState(false);
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [showUpdateModal, setShowUpdateModal] = useState(false);
    const [isAuthorized, setIsAuthorized] = useState(false); // Kontrola uprawnień
    ...
    const checkPermissions = () => {
        if (!token) {
            ...
            navigate('/login');  // Jeśli nie ma tokenu, przekieruj do logowania
            return;
        }

        if (user.isAdmin || supervisor) {  // Sprawdzamy, czy użytkownik jest administratorem lub nadzorcą
            setIsAuthorized(true);
        } else {
            setIsAuthorized(false);
            toast.error("You do not have permission to edit or delete this user!");
            navigate('/dashboard');  // Jeśli nie masz uprawnień, przekieruj na stronę główną
        }
    };

    // Używamy useEffect, aby sprawdzić uprawnienia po załadowaniu komponentu
    useEffect(() => {
        checkPermissions();
    }, [token]);
    ...
    // Jeśli użytkownik nie jest autoryzowany, nie wyświetlamy formularza
    if (!isAuthorized) {
        return <p className="text-center">You do not have permission to edit or delete this user.</p>;
    }
